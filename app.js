...
function compareUrlsUntilJpg(url1, url2) {
  ...
  if (path1.includes(".jpg") && path2.includes(".jpg")) {
    return (
      path1.substring(0, path1.indexOf(".jpg")) ===
      path2.substring(0, path2.indexOf(".jpg"))
    );
  }
...
async function extractItems() {
  ...
  for (let i = 0; i < 100; i++) {
    if (feedElements[i]) {
      const checkImage =
        feedElements && feedElements[i].querySelector("div:nth-child(8)");
...
async function mainBot() {
  try {
    ...
    await page.goto(
      formatGroup(configAccount.group_url) + "?sorting_setting=CHRONOLOGICAL",
      {
        timeout: 0,
        waitUntil: "networkidle2",
      }
          ...
          (item) => {
            ...
            for (let i = 0; i < feedElements.length; i++) {
              if (feedElements[i]) {
                ...
                if (foundImage) {
                  let commentBox = feedElements[i].querySelector(
                    "div[role='textbox']"
                  );
                  if (commentBox) {
      ...
      if (imgSource) {
        ...
        if (
          ![...commentedPosts].some((item) =>
            compareUrlsUntilJpg(item, imgSource)
          )
        ) {
          const commentPromise = (async () => {
            await page.type(
              `.item-${i} div[role='textbox']`,
              configAccount.comment
            );
            await page.keyboard.press("Enter");
